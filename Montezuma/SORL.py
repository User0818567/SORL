...
def test():
    ...
    while t>0:
        ...
        while not env.isTerminal() and episodeSteps <= maxStepsPerEpisode:
            ...
            if subgoal != -1:
                ...
                while not env.isTerminal() and numstate != goalnumstate and episodeSteps <= maxStepsPerEpisode:
                    ...
                    with torch.no_grad():
                        result = agent_list[option](batch_data, eps=0)
                    act = result.act[0]
        ...
        logx.msg("numstate trace: {}".format(numstate_trace))

    logx.msg("test rewards: {}".format(rewards))
    #test end
    data["test/mean_rew"].append(np.mean(rewards))
    data["test/var_rew"].append(np.std(rewards,ddof=1))
        
#start training
find_new_option = False
...
with tqdm.tqdm(total=args.episode_limit,desc = 'Train', **tqdm_config) as t:
    while episodeCount <= t.total:
        try:
            ...
            episodeSteps = 0
            predicate_loc, iskey = env.getSymbolicState()
            if episodeCount % 20 ==1 or find_new_option: #generate plan every 20 episode
                find_new_option = False
                tmp = 3
                while tmp>0:
                    tmp -= 1 
                ...
                if subgoal != -1:
                    ...
                    while not env.isTerminal() and numstate != goalnumstate and episodeSteps <= maxStepsPerEpisode:
                        ...
                        if numstate[0]==-1:
                            numstate = lastnumstate
                        if numstate!=lastnumstate:
                            metaAgent.getsubgoal(lastnumstate,numstate)
                            numstate_trace.append(numstate)
                        done = env.isTerminal()
