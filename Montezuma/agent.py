...
class SymbolicModel():
    def __init__(self, numStatepair=[], symbolicActions=[], gainreward=[],action2option = [],options = [], tabu=[]):
        super().__init__()
        self.numStatepair = numStatepair  # (numstate1,numstate2),numstate = [locidx,keyexist]
        #(eff+,eff-,precondition)
        self.symbolicActions = symbolicActions  # a list of (preconsition,effect)
        self.gainreward = gainreward  # a list of gainreward of each options and actions
        
        self.action2option = action2option
        ...
        self.explore = -1
        self.exploretimes = [0 for _ in range(Num_subgoal)]        

    def appendTabu(self,option):
        self.tabu.append(option)


    def reload(self,numStatepair,gainreward,options,success_ratio,action2option,exploretimes):
        self.numStatepair = numStatepair
        self.gainreward = gainreward
        self.options = options
        self.success_ratio = success_ratio
        self.action2option = action2option
        self.exploretimes = exploretimes
    ...
    def getoptions(self):
        return self.options,self.action2option

    def synchronization(self, state_dict):
        self.numStatepair = state_dict[0]  # (numstate1,numstate2),numstate = [locidx,keyexist,]
        self.symbolicActions = state_dict[1]  # a list of (preconsition,effect)
        self.gainreward = state_dict[2]
        self.tabu = state_dict[3]

    def add(self, action, reward):
        self.gainreward[action].append(reward)

    def getall(self):
        return [self.numStatepair, self.symbolicActions, self.gainreward, self.action2option,self.options, self.tabu]

    def getgoalnumstate(self, action):
        if action != -1:
            return self.numStatepair[action][1]

    def num2symbolic(self, numState):  # from numstate to symbolic state without quality
        symbolicState = []
        if numState[0] == -1:
            atpro = "(atNone)"
        ...
        if numState[1] == 1:
            ...
            symbolicState.append('(not (keyexist))')
        return symbolicState

    def getSymbolicEffect(self, state1, state2):
        effect = []
    ...
    # stepbuffer ia a list of numstates in a episode
    def getNumEffect(self,numstate1,numstate2):
        numeffect = []
        numeffect.append(numstate1[0])
        numeffect.append(numstate2[0])
        numeffect.append(numstate2[1]-numstate1[1])
        return numeffect
    ...
    def getaction2option(self,action):
        if action>=0 and action < len(self.action2option):
            return self.action2option[action]
        else:
            return Num_subgoal-1
    ...
    def generatesymbolicActions(self):
        self.symbolicActions.clear()
        explorereward = [0 for _ in range(Num_subgoal)]
        if self.explore>=0:
            explorereward[self.explore] = max(15.0 *(1- self.success_ratio[self.explore]),1)
        for i in range(len(self.numStatepair)):
            ...
            reward = explorereward[i]
            tmp += reward
            tmp = round(tmp)
            ss = "(increase (quality) " + str(tmp) + " )"
            effect.append(ss)
            self.symbolicActions.append((state1, effect, state2))

    def setSuccess(self,radio):
        self.success_ratio = radio
        flag = False
        for index in range(len(self.numStatepair)):
            if self.success_ratio[index] < 0.9 or self.exploretimes[index]<10:
                flag = True
                self.exploretimes[index]+=1
                self.explore = index
                break
        if flag==False:
            self.explore = len(self.numStatepair)-1

    def getExploreReward(self):
        explorereward = [0 for _ in range(Num_subgoal)]
        for index in range(Num_subgoal):
            if self.success_ratio[index] < 0.9:
                explorereward[index] = 15.0 *(1- self.success_ratio[index])
                break
        return explorereward

    # given symbolic option model,generate domain file
    def generateDomainFile(self):
        self.generatesymbolicActions()
        ...
        init_domain.close()
        for i in range(len(self.symbolicActions)):
            if i not in self.tabu:
                action = self.symbolicActions[i]
    ...
    def getsubgoal(self, numstate1, numstate2):
        if [numstate1, numstate2] in self.numStatepair:
            action = self.numStatepair.index([numstate1,numstate2])
            option = self.action2option[action]
            return action,option
        else:#create a new action
            numeffect = self.getNumEffect(numstate1,numstate2)
            self.numStatepair.append([numstate1,numstate2])
            self.gainreward.append([0])
            action = len(self.numStatepair)-1
            if numeffect in self.options:
                # exist corresponding option
                option = self.options.index(numeffect)
            else:
                #create a new option
                self.options.append(numeffect)
                option = len(self.options)-1
            self.action2option.append(option)
            return action,option
            
 

    # given current state and goal, generate problem.pddl
    def generateProblemFile(self, loc_pro, iskey, quality):
        ...
        init_prob.close()
        if self.explore>=0:
            exploregoal = self.num2symbolic(self.numStatepair[self.explore][1])
            for proposition in exploregoal:
                problemfile.write(proposition+"\n")
        problemfile.write("\n(> (quality) %s)\n))\n" % quality)
    ...
    def plan(self, loc_pro, iskey, quality):
        self.generateDomainFile()
        self.generateProblemFile(loc_pro, iskey, quality)
        self.generatePlan()
    ...
    def getsymbolicActions(self):
        return self.symbolicActions
