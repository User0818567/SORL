...
maxStepsPerOption = 5000
logx.initialize(logdir=args.hrllogdir, hparams=vars(args), tensorboard=True)

env = OfficeEnv(3)
subgoal2goal = [1,2,3]

agent_list = [Q_table(i,108,4,100000) for i in range(Num_subgoal-1)]
metaAgent = Q_table(-1,108*Num_subgoal+2,Num_subgoal-1,10000)


#record the random experience num of a subgoal
#option_t = [0 for _ in range(Num_subgoal)]
option_t = [411588,448102,0,0]
...
#load model
path_model = ["./sorldata1_2/policy_subgoal_0.pth","./sorldata2_2/policy_subgoal_0.pth"]

for i in range(2):
    with open(path_model[i],'rb') as f:
        data1 = pickle.load(f)
        agent_list[i].load(data1["table"],0.02)
